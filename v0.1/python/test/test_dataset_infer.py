...
sch_validate_path = os.path.abspath(os.path.join(os.path.realpath(__file__), "../../../test-examples/dataset/infer"))
...
for example in valid_schemas + invalid_schemas:
    test_name = "test_" + "_".join(example.replace("-", os.path.sep).split(os.path.sep)[3:])
