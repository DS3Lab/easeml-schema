...
def test_generator(example_file_src, positive_example=True):
    ...
    if positive_example:
        def test(self):
            ...
            expected_constant = match.src_dims is None or len(match.src_dims) == 0
...
sch_validate_path = os.path.abspath(os.path.join(os.path.realpath(__file__), "../../../test-examples/schema/match"))
...
for example in valid_schemas + invalid_schemas:
    test_name = "test_" + "_".join(example[:-len("-src.json")].replace("-", os.path.sep).split(os.path.sep)[3:])
